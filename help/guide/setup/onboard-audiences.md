...
Collaborationのオーディエンスの概念的な説明については、[&#x200B; オーディエンスの概要](../setup/audiences-overview.md)を参照してください。
...
>title="マーケティングアクション"
>abstract="<p>マーケティングアクションを使用して、Experience Platform から Real-Time CDP Collaboration に読み込むオーディエンスデータを制御します。 <strong>Data Collaboration</strong> マーケティングアクションは、C4、C5、C9、S1、S2、PSPD、およびRHD データ使用ラベルをサポートしています。 <strong>データサイエンス</strong>マーケティングアクションは、C9 データ使用ラベルをサポートしています。</p> <p> <ul><li> チェックボックスを<em>有効</em>にすると、Experience Platform で上記のラベルが付いているデータは除外され、Real-Time CDP Collaboration には取り込まれ<strong>ません</strong>。</li><li> チェックボックスを<em>無効</em>にすると、Experience Platform から Real-Time CDP Collaboration にすべてのデータがソースとされます。</li></ul></p>"
...
一部のプロファイル属性は、オブジェクトの配列であるリスト内でモデル化されます（ダイアログでは`Object[]`と表示されます）。 重要な識別子や属性は、顧客番号や車両識別子など、繰り返し使用されるレコードの中に存在します。

**[!UICONTROL プロファイル属性]** オプションで、**[!UICONTROL ソースフィールドを選択]** ダイアログのリストフィールドを展開し、一致キーにマッピングするネストされたフィールドを選択します。 例えば、`devices`を展開し、`Device ID`を選択します。 ネストされたフィールドは、そのマッピング行のソースフィールドに入力され、他のソースフィールドと同じようにターゲットフィールドにマッピングされます。

プロファイルがネストされたフィールドに複数の値を持つ場合、Collaborationは各値を個別に照合してカウントします。 事前にデータを統合したり、再構築したりする必要はありません。

![&#x200B; デバイス リストが展開され、マッピング用にデバイス ID フィールドが選択されたソース フィールドを選択ダイアログ &#x200B;](/help/assets/setup/add-manage-audiences/select-source-field-nested.png){zoomable="yes"}

>[!NOTE]
>
>リスト内にネストされたフィールドを選択するには、次の制限があります。
>
>* 一致キーに対してのみ、ネストされたフィールドを選択できます。 ネストされたフィールドを属性として選択することはサポートされていません。
>* リストごとに1つのネストされたフィールドを一度に選択できます。
>* オブジェクトのリストではなく、単純な値のリストがダイアログに表示されますが、それらの値を選択することはできません。
...
| **[!UICONTROL ソース]** | オーディエンスのソース元を示します。 |
