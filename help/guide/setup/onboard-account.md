...
>abstract="ECID や IP アドレスなどのファーストパーティデバイス ID は、複数の個人間で共有される可能性のあるデバイスに直接接続されます。"
...
* **[!UICONTROL IDFA]**:Apple iOS デバイスで使用される広告主（IDFA）の識別子
* **[!UICONTROL GAID]**:Google デバイスで使用されるAndroid広告主 ID
...
>[!NOTE]
>
>Adobeと [!DNL AdFixus] の統合により、各アカウントの一意の [!UICONTROL AdFixus ID] が、Adobeでエンコードされた共通の形式にマップされます。 これらのマッピングは、共同作業者間の重複を識別するために使用されます。 **[!UICONTROL AdFixus ID]** を使用してオーディエンスをアクティブ化する場合、元の ID が使用されます。 Adobeでエンコードされた形式は、Collaborationから移動されません。
