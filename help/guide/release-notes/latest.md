...
Real-Time CDP Collaborationでは、オーディエンスをソーシングするための新しい手段として CSV ファイルのアップロードがサポートされるようになりました。また、オーディエンスのマッチングと測定を強化する新しいモバイル一致キー（IDFA および GAID）もサポートされるようになりました。

**新機能または更新された機能**

| 機能 | 説明 |
| ------- | ----------- |
| オーディエンスソーシング用の CSV アップロード | CSV ファイルをソースオーディエンスに UI から直接Collaborationにアップロードします。 短期間のコラボレーションプロジェクトでのファーストパーティデータのオンボーディングに最適です。 詳しくは、[&#x200B; オーディエンスソーシングガイド用の CSV ファイルのアップロード &#x200B;](../setup/upload-csv-audience-sourcing.md) を参照してください。 |
| Mobile Match キーのサポート | Collaborationでは、オーディエンスのマッチングおよび測定用に、IDFA や GAID などのモバイルマッチキーをサポートするようになりました。 これらの一致キーは、アカウントの設定時に選択され、新しい接続の接続設定を設定する際やダウンストリームのコラボレーションワークフローで使用できます。 詳しくは、[&#x200B; マッチキーの設定ガイド &#x200B;](../setup/onboard-account.md#set-up-match-keys) を参照してください。 |
...
>[!IMPORTANT]
>
>複数の一致キーが使用されているオーディエンスをアクティブ化する場合、1 つ（または複数）の一致キーに重複がない、オーディエンスサイズがない、またはしきい値を下回る場合、アクティベーション全体が失敗します。 オーディエンスに十分な重複があり、アクティブ化する前にすべての一致キーで 1000 個の ID の最小しきい値を満たしていることを確認します。
...
* オーディエンスインデックススコアがプロジェクトの **[!UICONTROL 検出]** タブで使用できるようになりました。 オーディエンスインデックススコアは、オーディエンスが共同作業者のオーディエンスにどの程度適合しているかを測定する指標です。 このスコアは、基になるオーディエンスサイズと重複に基づいて計算されます。 オーディエンスインデックススコアについて詳しくは、[&#x200B; オーディエンスインデックススコア &#x200B;](../collaborate/discover.md#audience-index-score) ガイドを参照してください。
