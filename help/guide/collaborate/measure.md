...
このビューでは、表形式で測定するために選択したコンバージョンイベントの詳細な分類を提供します。 結果は&#x200B;**キャンペーン ID** レベルで計算されます。 テーブルには次のものが含まれます。

* **コンバージョンイベント**：追跡している各コンバージョンイベントの名前。
* **コンバージョン数**: キャンペーン IDの各イベントで発生したコンバージョンの合計数。
* **コンバージョン値**: キャンペーン IDの各コンバージョンイベントに起因する値。
...
### プレースメント別コンバージョン数 {#conversions-by-placement}

この表は、アトリビューションレポート用に選択されたコンバージョンイベントの&#x200B;**配置ID レベルの内訳**&#x200B;を示しています。 1つのキャンペーン IDに複数のプレースメント IDを含めることができるので、このビューを使用して、キャンペーン内のコンバージョンがどのように分類されるかを確認します。 選択したコンバージョンイベントごとに、各プレースメントに関連付けられたコンバージョンと、対応するコンバージョン値が表に表示されます。 テーブルには次のものが含まれます。

* **プレースメント名またはID**: クリエイティブが実行されたプレースメントの識別子。
* **コンバージョン数**: レポートの作成時に選択した各コンバージョンイベントについて、そのプレースメントに起因するコンバージョンの数。 プレースメントには、レポートごとに許可されるコンバージョンイベントの最大数に一致する、最大3つのコンバージョンイベントを表示できます。
* **コンバージョン値**：特定のプレースメントに対する各コンバージョンイベントに起因する値。

プレースメント別![ コンバージョン数。](/help/assets/collaborate/measure/conversions-by-placement.png)
...
日数&#x200B;]**で**[!UICONTROL  ルックバックウィンドウを更新するには、数値を入力するか、増減オプションで調整します。 続いて、レポートを作成するコンバージョンイベントを選択します。 利用可能なリストから最大&#x200B;**3**&#x200B;個のコンバージョンを選択できます。
